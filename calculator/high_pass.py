...
# Loads some common capacitor and resistor values and then exhaustively searches the space of possibilities to find a good combination.
...
def main() :
   ...
   resValues = [x for x in resValues if (x >= convertToValue("200") and x <= convertToValue("70K")) ]
   capValues = [x for x in capValues if (x >= convertToValue("200nF") and x <= convertToValue("300nF")) ]
   ...
   for R1 in resValues1:
      for R2 in resValues2:
         for C1 in capValues:
            C2 = C1
            try:
               q = math.sqrt(C1*C2*R1*R2)/(R1*(C1+C2))
